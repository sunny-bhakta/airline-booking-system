...
export class BookingsController {
  ...
  @Get(':id/payments')
  @ApiOperation({ summary: 'Get payment information for a booking' })
  @ApiParam({ name: 'id', description: 'Booking UUID' })
  @ApiResponse({
    status: 200,
    description: 'Payment information including transactions, invoices, and receipts',
  })
  @ApiResponse({ status: 404, description: 'Booking not found' })
  getPaymentInfo(@Param('id') id: string) {
    return this.bookingsService.getPaymentInfo(id);
  }

  @Get(':id/has-payment')
  @ApiOperation({ summary: 'Check if booking has completed payment' })
  @ApiParam({ name: 'id', description: 'Booking UUID' })
  @ApiResponse({
    status: 200,
    description: 'Returns true if booking has completed payment',
  })
  @ApiResponse({ status: 404, description: 'Booking not found' })
  hasPayment(@Param('id') id: string) {
    return this.bookingsService.hasPayment(id);
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'All ancillary services (baggage, in-flight services, travel insurance) with totals',
  })
  @ApiResponse({ status: 404, description: 'Booking not found' })
  getAncillaryServices(@Param('id') id: string) {
    return this.bookingsService.getAncillaryServices(id);
  }

  @Get(':id/baggage')
  @ApiOperation({ summary: 'Get baggage for a booking' })
  @ApiParam({ name: 'id', description: 'Booking UUID' })
  @ApiResponse({ status: 200, description: 'List of baggage items' })
  @ApiResponse({ status: 404, description: 'Booking not found' })
  getBaggage(@Param('id') id: string) {
    return this.bookingsService.getBaggage(id);
  }

  @Get(':id/in-flight-services')
  @ApiOperation({ summary: 'Get in-flight services for a booking' })
  @ApiParam({ name: 'id', description: 'Booking UUID' })
  @ApiResponse({ status: 200, description: 'List of in-flight services' })
  @ApiResponse({ status: 404, description: 'Booking not found' })
  getInFlightServices(@Param('id') id: string) {
    return this.bookingsService.getInFlightServices(id);
  }

  @Get(':id/travel-insurance')
  @ApiOperation({ summary: 'Get travel insurance for a booking' })
  @ApiParam({ name: 'id', description: 'Booking UUID' })
  @ApiResponse({ status: 200, description: 'List of travel insurance policies' })
  @ApiResponse({ status: 404, description: 'Booking not found' })
  getTravelInsurance(@Param('id') id: string) {
    return this.bookingsService.getTravelInsurance(id);
  }
